...
import styles from "./VerbalTest.module.css";
...
const VerbalTest = () => {
  ...
  useEffect(() => {
    fetch("https://opentdb.com/api.php?amount=20&category=10&type=multiple")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleSubmit = () => {
    ...
    setScores(prev => ({ ...prev, verbal: newScore }));
  };
  ...
  return (
    <div className={styles.container}>
      ...
      {/* ✅ Header sirf tab dikhe jab test submit nahi hua ho */}
      {!submitted && (
        <div className={styles.header}>
          <h2>Verbal Reasoning Test</h2>
      ...
      {timeLeft === 0 && !submitted && (
        <div className={styles.timeOverMsg}>
          Your time is over. Test auto-submitted!
        </div>
      )}
    </div>
  );
